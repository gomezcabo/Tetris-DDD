...
export function getCellValues(board: Board): BoardCellValue[] {
  return Array.from({ length: BOARD_ROWS * BOARD_COLS }).map((_, index) =>
    getCellValue(board, Math.floor(index / BOARD_COLS), index % BOARD_COLS)
  );
}
...
export function lockBlock(board: Board): Board {
  const newBoard = cloneBoard(board);
  ...
  // Lock the current block on its final position making it part of the board cells
  for (let row = 0; row < board.currentBlock.shape.length; row++) {
  ...
  newBoard.cells.forEach((row, rowIndex) => {
    if (row.every((col) => col !== null)) rowsToClear.push(rowIndex);
  });

  const emptyRows = Array(rowsToClear.length).fill(Array(BOARD_COLS).fill(null));
  const boardWithoutCompletedRows = newBoard.cells.filter((_, index) => !rowsToClear.includes(index));

  newBoard.cells = [...emptyRows, ...boardWithoutCompletedRows];
  newBoard.score += rowsToClear.length;
